...
export function mapHttpError(status, result = {}) {
  ...
  if (/IP .+ is Not Allowed/i.test(detail)) {
    return (
      `${detail} ` +
      "Ask IntouchPay to whitelist this server IP for live API. " +
      "Vercel IPs change unless you enable Static IPs, so a one-time whitelist of a single address will break again."
    );
  }
  ...
  const base = HTTP_ERROR_MESSAGES[status] || "IntouchPay request failed";
