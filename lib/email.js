...
export async function sendBookingReceivedEmail({
  ...
  const html = baseTemplate({
    ...
    bodyHtml: `
      <p>Hello ${customerName || "Guest"},</p>
      <p>Thank you! We received your reservation request${
        payLater ? " and will confirm it shortly." : "."
      }</p>
      <ul>
        <li><strong>Booking:</strong> ${orderId}</li>
        <li><strong>Room:</strong> ${roomTitle || "—"}</li>
        <li><strong>Check-in:</strong> ${checkIn}</li>
        <li><strong>Check-out:</strong> ${checkOut}</li>
        <li><strong>Total:</strong> RWF ${Number(amount).toLocaleString()}</li>
  ...
  try {
    return await deliverEmail({
      to,
      subject: `${companyName()} — Booking received (${orderId})`,
      html,
    });
  } catch (error) {
    logError("email.booking_received.failed", error, { orderId });
    return { ok: false, error: error.message };
  }
}
...
export async function sendNewBookingAdminEmail({
  ...
  const pickupHtml = pickupRequested
    ? `<li><strong>Car pickup:</strong> Yes — ${pickupDetails || "details pending"}</li>`
    : `<li><strong>Car pickup:</strong> No</li>`;
  const requestsHtml = specialRequests
  ...
  const html = baseTemplate({
    ...
    bodyHtml: `
      <p>A new booking was submitted on the website.</p>
      <ul>
        <li><strong>Booking:</strong> ${orderId}</li>
      ...
      <p><a href="${getAppUrl()}/admin/bookings">Open admin bookings</a></p>
    `,
  });
  ...
  try {
    return await deliverEmail({
      to,
      subject: `${companyName()} — New booking ${orderId}`,
      html,
    });
  } catch (error) {
    logError("email.new_booking_admin.failed", error, { orderId });
    return { ok: false, error: error.message };
  }
}

export async function sendBookingRejectedEmail({ to, customerName, orderId, roomTitle }) {
  const html = baseTemplate({
    title: "Booking update",
    bodyHtml: `
      <p>Hello ${customerName || "Guest"},</p>
      <p>We regret to inform you that booking <strong>${orderId}</strong>${
        roomTitle ? ` for <strong>${roomTitle}</strong>` : ""
      } could not be confirmed.</p>
      <p>Please contact us if you have questions or would like to choose different dates.</p>
      <p><a href="${getAppUrl()}/book">Search availability</a></p>
    `,
  });
  ...
  try {
    return await deliverEmail({
      to,
      subject: `${companyName()} — Booking update (${orderId})`,
      html,
    });
  } catch (error) {
    logError("email.booking_rejected.failed", error, { orderId });
    return { ok: false, error: error.message };
  }
}
