...
export async function markPaymentTransactionFailed(id, message = "") {
  if (!id) return null;
  return prisma.paymentTransaction.update({
    where: { id },
    data: {
      status: "FAILED",
      message: String(message || "Payment request failed").slice(0, 500),
    },
  });
}

/** Expire stuck PENDING rows so the guest can enter the phone again. */
export async function getActivePendingTransactionForOrder(orderId) {
  ...
  if (Number.isFinite(ageMs) && ageMs > STALE_PENDING_MS) {
    await markPaymentTransactionFailed(pending.id, "Pending payment expired. Please try again.");
    return null;
  }
  return pending;
}
