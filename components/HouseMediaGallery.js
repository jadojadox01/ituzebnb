...
export function HouseMediaGallery({ listing, canBook }) {
  ...
  return (
    <div className="space-y-4">
      <div className="relative overflow-hidden rounded-lg border border-border bg-muted shadow-smooth">
        {activeImage ? (
          <>
            ...
            <div className="absolute inset-x-0 bottom-0 flex flex-wrap items-end justify-between gap-2 bg-gradient-to-t from-black/60 to-transparent p-3 sm:gap-3 sm:p-4">
              {canBook ? (
                <Link href="/register" className="focus-ring inline-flex items-center gap-2 rounded-md bg-secondary px-3 py-2.5 text-xs font-extrabold text-secondary-foreground shadow-sm transition hover:brightness-105 sm:px-4 sm:py-3 sm:text-sm">
                  <CalendarCheck size={16} aria-hidden="true" />
                  {t("bookNow")}
                </Link>
              ) : (
                <span className="rounded-md bg-white/95 px-3 py-2.5 text-xs font-bold text-muted-foreground sm:px-4 sm:py-3 sm:text-sm">{tStatus(listing.status, t)}</span>
              )}
              {images.length > 1 && (
          ...
          <div className="flex aspect-[16/10] items-center justify-center p-8 text-center text-sm text-muted-foreground">
            {t("noRoomPhotos")}
          </div>
        )}
      </div>
      ...
      <button
        type="button"
        className="focus-ring flex w-full items-center justify-between rounded-lg border border-border bg-card p-4 text-left shadow-sm"
        disabled={!listing.video_url}
      >
        <span>
          <span className="block font-extrabold">{t("videoTour")}</span>
          <span className="mt-1 block text-sm text-muted-foreground">
            {listing.video_url ? t("videoTourAvailable") : t("videoNotAvailable")}
          </span>
        </span>
        {listing.video_url ? (
      ...
      {listing.video_url ? (
        <div className="overflow-hidden rounded-lg border border-border bg-black">
          <video src={listing.video_url} controls className="w-full" preload="metadata">
